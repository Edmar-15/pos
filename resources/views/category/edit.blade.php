...
<body class="flex h-screen bg-gray-100">
    ...
    <main class="flex-1 p-6 overflow-auto">
        ...
        <div class="p-4 bg-white rounded shadow">
            ...
            <input type="text" id="categoryName" value="{{ $category->name }}" placeholder="Enter category name" class="form-input">
            <button id="updateCategoryBtn" class="btn-primary mt-2.5">Update Category</button>
        </div>
    </main>
    ...
    <script>
        document.getElementById('updateCategoryBtn').addEventListener('click', function() {
            ...
            .then(res => res.json())
            .then(data => {
                // Store updated category in sessionStorage
                let editedCats = JSON.parse(sessionStorage.getItem('editedCategories')) || [];
                editedCats.push(data.category);
                sessionStorage.setItem('editedCategories', JSON.stringify(editedCats));

                // Redirect back to category table page
